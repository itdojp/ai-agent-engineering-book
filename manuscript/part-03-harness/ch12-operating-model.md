...
- merge から production 確認、停止、rollback、再開までの gate を設計できる
...
| Production Delivery | production confirmation rate/latency、marker mismatch、rollback recovery time | merge 後の公開確認と復旧が閉じているか |
...
重要なのは、指標を増やすことではなく、指標に対する行動を決めることだ。verify failure rate が高ければ prompt や brief の質を疑う。PR cycle time が長ければ review budget を疑う。review completion rate が低い、または unresolved review thread residual が残るなら、merge 前 gate と PR template を疑う。production confirmation rate が 100% 未満、marker mismatch または post-deploy metric regression が発生した場合は、merge 済みでも完了を止める。trace coverage が低ければ failure analysis の材料不足を疑う。evidence freshness failure が多ければ reviewer が current-run verify を確認できていないことを疑う。stale docs count と hygiene backlog age が悪化したら cleanup 専用 work package を先に開く。metrics は throughput 自慢ではなく、queue 診断、failure analysis、review quality 改善に使うべきである。
...
production へ影響する変更は、merge 前に target/marker/metric/owner/rollback を決め、merge 後に対象 SHA の deployment、HTTP、semantic marker、metric を別々に確認する。
...
公開変更では、lead が merge 判断、operator が deploy 証跡の収集、reviewer が review completion、Delivery Owner が production confirmation を担当する。1 人運用なら同一人物が兼任しても、判断記録は分ける。成功 scenario では対象 SHA の deployment success、root/代表 route の HTTP 200、semantic marker 一致、metric 閾値内を確認する。deploy failure/unknown なら完了を止める。marker または metric 回帰なら revert PR で rollback し、新しい main SHA の公開確認まで行う。
...
- review completion、deployment approval、deployment success、production confirmation を分離し、対象 SHA の HTTP、semantic marker、metric で完了を判定する。
