...
artifacts:
  ...
  - docs/guardrail-coverage-matrix.md
...
- AI / 外部サービス投入と tool surface ごとの guardrail coverage / blind spot を説明できる
...
この repo では、single-agent harness を次の 7 要素で構成する。
...
| external input boundary | AI / 外部サービス投入前の分類、redaction、provider 条件、guardrail coverage をsurface別に確認する | `docs/guardrail-coverage-matrix.md`、permission policy、`checklists/verification.md` |
...
runtime-managed loop で十分か、repo-owned manual harness を厚く残すべきかは、次の表で判断するとよい。前提として、最終 review / merge は常に human-owned であり、runtime が execution state を管理していても source-of-truth artifact は repo-owned のままである。

| 判断要因 | runtime-managed loop で十分な条件 | repo-owned manual harness を厚く残す条件 |
|---|---|---|
| human approval | 最終 review 以外に追加の approval gate がない | 実行前 / 実行中 / 実行後に human approval が必要 |
| evidence / audit trail | runtime の status と current-run verify で review に足りる | custom evidence bundle や監査用の記録を task ごとに残す必要がある |
| stop / resume logic | 線形な run と単純な retry で閉じる | 条件分岐した stop / resume、retry 分類、handoff を repo artifact で固定したい |
| source of truth | task brief、runbook、done criteria が固定済みで run 中に揺れない | artifact sync、refresh policy、owned files を run 中も明示的に更新したい |
| review responsibility | human reviewer が runtime の結果をそのまま読めば足りる | review 用の `Changed Files`、`Verification`、`Remaining Gaps` を明示的に整形したい |

つまり、runtime-managed loop は mechanism が足りるかどうかの判断であり、責務が消えるかどうかの判断ではない。approval、evidence、stop / resume、artifact sync のどれかが custom になる時点で、repo-owned manual harness を薄くしすぎない方が安全である。
...
#### AI / 外部サービス投入と tool guardrail

AI / 外部サービス投入は、secret だけを避ければ安全になるわけではない。Issue、PR、log、eval case、trace、evidence bundle には、顧客情報、個人情報、要配慮情報、未公開仕様、脆弱性情報、組織内部の判断過程が混ざりうる。外部 API、hosted tool、web search、tracing、eval judge へ投入する前に、データ分類、redaction、provider の retention / training use / logging 条件、保存先、承認者を確認する。

この確認を `external input boundary` として扱う。external input boundary は、tool call の可否だけでなく、何をどの粒度で渡すか、出力をどこへ保存するか、trace に何を残すかまで含む。guardrail はこの境界の前後で機密値、個人情報、scope 外 tool call、approval 必須操作を止めるために置く。ただし、guardrail は最終 review、current-run verify、human approval の代替ではない。

`docs/guardrail-coverage-matrix.md` は、この境界を external input、model output、tool definition、tool call、tool execution、tool result、resource、trace、session、external service、protocol prompts / roots、protocol sampling / elicitation に分ける。各 surface で classification、redaction、permission、approval、storage / retention、verification を独立に確認する。特に、tool definition、tool result、server prompt、resource content は信頼済み命令ではなく外部入力である。副作用は tool call の検査だけで終わらせず、実行直前の approval、least privilege、postcondition、rollback evidenceで閉じる。

| Surface | 最低限のcontrol | 代表的なblind spot |
|---|---|---|
| external input | provenance、classification、redaction | chain途中のretrievalやtool resultは入口guardrailの外側になりうる |
| model output | destination別の再分類、schema / fact確認 | 中間agent出力やtool引数は最終output検査の外側になりうる |
| tool definition | trusted registry、version、capability確認 | description / annotation自体がinstruction injectionを含みうる |
| tool call | 引数、path、host、payloadのpermission確認 | guardrailをattachしていないtool typeへ同じcoverageを仮定しやすい |
| tool execution | least privilege、実行直前approval、postcondition | 引数検査だけではOS権限や不可逆な副作用を止められない |
| tool result | untrusted inputとして再分類、provenance確認 | stdout、web、MCP resultが次の命令として混入しうる |
| resource | path / domain / URI allowlist、integrity / freshness確認 | fileやresource取得がtool-call guardrailと別経路になりうる |
| trace / log | redaction、viewer制限、retention、leakage scan | tracing SDK、CI log、artifact uploadは自動検査されない場合がある |
| session / memory | task境界、TTL、refresh / invalidate、source-of-truth確認 | 保存済みsecretやstale contextは入口検査だけでは消えない |
| external service | provider / purpose / region、retention / training use確認 | local guardrailはprovider側の保存やsubprocessorを強制できない |
| protocol prompts / roots | server / prompt / root allowlist、prompt provenance、host ACL / sandbox | prompt discoveryはinstruction trustを保証せず、root通知だけではfilesystem境界をenforceしない |
| protocol sampling / elicitation | request / destination表示、明示approval、field制限 | protocolはhostのconsent UIやpolicy enforcementを自動保証しない |

coverage の主語も明確にする。たとえば input guardrail が chain の入口だけ、output guardrail が最終 agent だけ、tool guardrail が attach 済み function tool だけを対象にする runtime では、retrieval、hosted tool、MCP resource、中間 agent、trace、session は別 control が必要になる。利用中 runtime の公式文書で coverage を確認し、不明または適用外の surface は permission policy で `deny` か `escalate` にする。

minimum walkthrough は `evals/guardrail-surface-cases.json` に固定する。hostile input、hostile tool output、tainted tool metadata、unsafe side effect、trace leakage、stale / sensitive session、external provider boundaryの各caseについて、必要control、期待判定、verification evidenceを確認する。`scripts/check-guardrail-coverage.py` は必須surfaceやcontrolを落としたfixtureをrejectするため、表だけ更新して検証契約を空洞化させることを防ぐ。
...
coding agent に仕事を最後までさせるには、次の 7 要素が artifact と command で明確になっている必要がある。

1. init が 1 command で明確である
2. owned files を含む work boundary が artifact 化されている
3. approval が必要な操作を止める permission policy がある
4. AI / 外部サービス投入前に external input boundary と guardrail coverage を確認できる
5. done / blocked / needs-human-approval を分ける done criteria がある
6. 完了前に通す verify command が固定されている
7. Exit State、Changed Files、Verification、Remaining Gaps を含む report format が固定されている

retry と rollback は、この 7 要素を崩さずに failure mode ごとへ適用する運用ルールである。この 7 要素がないと、agent は途中で止まるか、逆に止まるべき場所で止まれない。前者は「停止が早い」失敗であり、後者は「壊す」失敗である。CH01 の failure model を、Harness Engineering の側から防いでいると考えるとよい。

逆に、task brief、context pack、skill が揃っていても、init / work boundary / permission policy / external input boundary / done criteria / verify command / report format が曖昧なら、agent は最後の 10% で崩れる。実務で問題になるのはこの最後の 10% である。runtime-managed capability が増えても、その 10% を閉じる責務は repo / team 側に残る。Harness Engineering は、その 10% を偶然ではなく artifact で支える。
...
AI / 外部サービスへ投入する issue、PR、log、evidence は分類、redaction、provider 条件を確認してから扱う。
...
この修正版では、runtime-managed capability の有無に関係なく、single-agent harness の init、work boundary、permission policy、external input boundary、done criteria、verify command、report format が artifact と command で固定されている。

比較観点:
- bad は runtime capability を harness の代わりにしている
- bad は approval 境界、外部投入境界、verify ルールが未定義である
- good は init、work boundary、permission policy、external input boundary、done criteria、verify command、report format を 1 つの実行枠として定義している
...
3. 境界確認
   ...
   - verify script を変えない
   - AI / 外部サービス投入は行わず、必要になった場合は external input boundary と permission policy で止める
...
2. hostile input、tool output、trace leakageを含むcoverage matrixを作り、permission policyとescalation ruleへ接続する。
...
- `scripts/check-guardrail-coverage.py`
  必須surface、control、minimum caseの欠落をrejectするdeterministic verifierとして読む。
- `docs/guardrail-coverage-matrix.md`
  tool-independent contract、surface別control、vendor / protocol profile、blind spotを確認する正本である。
- `evals/guardrail-surface-cases.json`
  hostile input、tool output、side effect、trace、session、provider boundaryのwalkthroughを機械可読に固定する。
- `sample-repo/docs/harness/single-agent-runbook.md`
...
- この章を探し直すときは、まず `scripts/init.sh`、`docs/guardrail-coverage-matrix.md`、`evals/guardrail-surface-cases.json`、`sample-repo/docs/harness/single-agent-runbook.md`、`sample-repo/docs/harness/permission-policy.md`、`sample-repo/docs/harness/done-criteria.md` を正本として見る。single-agent harness は prompt の言い換えではなく、runtime-managed capability と独立した init、work boundary、permission policy、external input boundary、done criteria、verify command、report format の束である。
...
- single-agent harness の最小構成は、init、work boundary、permission policy、external input boundary、done criteria、verify command、report format である。retry rule は、この構成を前提に failure mode ごとへ適用する運用ルールである。
