...
| External input overexposure | issues, PRs, logs, traces, or evidence enter an AI or external service without classification | personal data, unpublished specs, vulnerability details, or internal decisions leak even when no secret is present | place an external input boundary and confirm redaction, provider terms, approval, and storage location |
| Guardrail blind spot | the team assumes guardrails cover every hosted tool, function tool, trace, and session | unsafe tool calls or data submissions pass through uncovered surfaces | confirm guardrail coverage per tool surface and stop uncovered areas with the permission policy and human approval |
