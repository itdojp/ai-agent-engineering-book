...
## Purpose
support-hub で coding agent を single-agent harness として動かすときの標準起動手順、作業境界、retry rule、exit rule を定義する。コマンドは repo root で実行する前提とする。runtime が background execution、hosted tools、managed context を持っていても、この runbook が定義する start / stop / report contract は維持する。
...
## Runtime Notes
- managed context は read 内容の保持を助けても、task brief・permission policy・done criteria の代替にはならない
- hosted tool を使えても、scope 外変更や approval 境界の判断は repo artifact に従う
- background execution を使えても、verify 後の report format は省略しない
- 最終 review / merge は常に human-owned であり、runtime の green status は repo 側の done criteria を置き換えない

## Loop Choice Rule

| 判断要因 | runtime-managed loop で十分 | manual harness を厚く残す |
|---|---|---|
| human approval | 最終 review 以外の追加 approval がない | 実行途中または verify 後に approval 境界がある |
| evidence | runtime の status と verify で足りる | task ごとに custom evidence / audit trail が必要 |
| stop / resume | 線形な run と単純な retry で閉じる | conditional stop、resume、handoff を明示したい |
| source artifacts | task brief / permission policy / done criteria が固定済み | run 中に artifact sync や refresh policy を更新したい |

どちらの列でも source of truth は repo artifact に残る。runtime が execution state を持っていても、manual harness が不要になるとは限らない。
...
- runtime の status 表示が green でも、repo 側の done criteria を満たさなければ完了扱いにしない。

## Report Contract
- canonical な報告項目は `Changed Files`、`Verification`、`Remaining Gaps` とする。
- Exit State は別見出しを増やさず、報告の先頭で 1 行として明示する。
