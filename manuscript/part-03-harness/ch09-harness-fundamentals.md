...
| report format | exit state、changed files、verification を残す | runbook と done criteria |
...
owned files は `src/support_hub/service.py` と `tests/test_service.py` に限定する。
...
返答は Exit State, Changed Files, Verification, Remaining Gaps を含める。
```
...
5. 終了判定
   - verify が通り、Root Cause を説明でき、Changed Files / Verification / Remaining Gaps を report できるなら `done`
