...
Prompt Contract が単一タスクの入力と出力を固定するのに対し、context artifact は判断材料を固定する。両者を混ぜると、prompt が肥大化し、progress note が source of truth を上書きし、context pack が古い要件を運ぶ。そこで task brief、Progress Note、context pack を別 artifact として持つ。
...
Progress Note や context pack が task brief より強い source of truth になると、古い判断が残りやすい。Context Engineering の実務では、「何を書くか」より「何を書きすぎないか」の方が重要である。
