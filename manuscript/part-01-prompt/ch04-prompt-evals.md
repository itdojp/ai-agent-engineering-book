...
status: draft
artifacts:
  - evals/prompt-contract-cases.yaml
...
`evals/prompt-contract-cases.yaml` では、`feature-contract` と `bugfix-contract` に対して 5 件のケースを定義した。たとえば次の観点が入っている。
...
- `evals/prompt-contract-cases.yaml` のケース定義が壊れていないか
...
```text
`evals/prompt-contract-cases.yaml` にある 5 件を固定の eval set とする。
...
そこで v2 では、`Missing Information Policy`、`docs と tests を同時に更新する`、`仕様外の UI / API 変更をしない` を明示した。この変更を評価するために、`evals/prompt-contract-cases.yaml` のうち次の 3 ケースを見る。
