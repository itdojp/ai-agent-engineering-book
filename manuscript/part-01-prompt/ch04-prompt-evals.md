...
- model、API、SDK、tool docs の確認日

これができると、「なぜ先週は通って今週は落ちたか」を説明できる。versioning の対象は prompt だけではなく、workflow contract 全体である。

`model/runtime profile` は、単なるモデル名のメモではない。比較対象の model、API family、SDK、runtime、tool set、approval policy、確認日を 1 組として固定する記録である。モデル名、API、SDK、vendor 固有機能を本文や PR で扱うときは、利用中の official docs をその時点で確認し、確認日を残す。特に tool use、structured output、session / memory、guardrail、tracing、eval framework は product surface の変化が速い。ここを曖昧にすると、eval の差分が prompt 改善によるものか、runtime 変更によるものか説明できない。

比較運用では「最新モデルなら良い」と書かない。採用判断は、同じ eval suite と rubric を、記録済みの model/runtime profile に対して再実行して行う。モデルや SDK を変えたら、品質、tool-call、cost / latency、approval signal、trace / evidence の残り方をまとめて見る。model/runtime profile が変わったのに eval を再実行していない PR は、review-ready ではなく追加検証待ちである。
...
2. 同じ feature task に対して prompt v1 / v2 を比較する記録テンプレートを作りなさい。case ごとの pass/fail、重大 fail、観察メモ、model/runtime profile、公式 docs の確認日を残せる形にすること。
