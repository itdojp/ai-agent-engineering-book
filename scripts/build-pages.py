...
def page_chrome(page: Page, body: str, current_search_placeholder: str) -> str:
    ...
    return (
        ...
        "if(!cb||!toggle)return;"
        "function revealActiveToc(){"
        "if(!cb.checked)return;"
        "var sidebar=document.getElementById('sidebar');"
        "var active=sidebar?sidebar.querySelector('.toc-link.active'):document.querySelector('.toc-link.active');"
        "if(!sidebar||!active)return;"
        "var sidebarRect=sidebar.getBoundingClientRect();"
        "var activeRect=active.getBoundingClientRect();"
        "var target=sidebar.scrollTop+(activeRect.top-sidebarRect.top)-Math.max(16,(sidebar.clientHeight-activeRect.height)/2);"
        "sidebar.scrollTop=Math.max(0,target);"
        "}"
        "function scheduleActiveToc(){if(window.requestAnimationFrame){window.requestAnimationFrame(revealActiveToc);}setTimeout(revealActiveToc,80);}"
        "function syncSidebarState(){toggle.setAttribute('aria-expanded',cb.checked?'true':'false');if(cb.checked){scheduleActiveToc();}}"
