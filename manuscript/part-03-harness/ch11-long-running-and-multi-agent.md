...
| plan | work package が 1 本で足りる | workstream ごとに checkpoint が要る | `sample-repo/tasks/FEATURE-002-plan.md` |

このカードで見るべき点は、「複雑だから multi-agent」ではなく、「分割後に統合コストが下がるか」である。feature list と restart packet が弱い状態で agent を増やすと、単に state 崩壊を並列化するだけになる。逆に plan と role split が明確なら、multi-agent は long-running task の coordination cost を下げる。
...
- `sample-repo/docs/harness/restart-protocol.md`
  restart packet と restart steps を確認する。再開時に何を source of truth に戻すかの本文対応箇所で使う。
- `sample-repo/docs/harness/multi-agent-playbook.md`
...
- この章を探し直すときは、まず `sample-repo/docs/harness/feature-list.md`、`sample-repo/docs/harness/restart-protocol.md`、`sample-repo/docs/harness/multi-agent-playbook.md`、`sample-repo/tasks/FEATURE-002-plan.md` を正本として見る。multi-agent は role split と restart packet が揃って初めて使う。
