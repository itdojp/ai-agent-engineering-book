...
### 3. restart protocol
restart protocol は、中断後に「前回の続きらしいこと」を推測で始めないための手順である。`sample-repo/docs/harness/restart-protocol.md` では、canonical inputs、live checks、restart steps、handoff contract を明示している。

canonical inputs の最低入力は次の 5 つでよい。
...
`FEATURE-002-plan.md` では、Track B が `service.py` と `tests/test_service.py` を持ち、Track C が `models.py` と `store.py` を持つ。テストや docs の統合は reviewer / verifier フェーズへ送る。つまり、multi-agent の要点は「何を同時に進めるか」より先に、「何を同時に進めないか」を決めることである。
...
中断時は `sample-repo/docs/harness/restart-protocol.md` の canonical inputs と live checks を更新する。
...
- `sample-repo/docs/harness/restart-protocol.md`
  canonical inputs、live checks、handoff contract を確認する。再開時に何を source of truth に戻すかの本文対応箇所で使う。
- `sample-repo/docs/harness/multi-agent-playbook.md`
