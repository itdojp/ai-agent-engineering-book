...
`sample-repo/docs/harness/restart-protocol.md` では、plan、feature list、owned files、merge order、最新 `Progress Note`、verify evidence、open questions から成る `restart packet` を定義している。restart protocol がないまま multi-agent を始めると、役割分担より先に state が壊れる。
