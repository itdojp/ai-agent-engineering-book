...
This is where ChatGPT is useful. The goal is not to treat the conversation as the deliverable. The goal is to use exploratory dialogue to surface missing decisions and then converge on artifacts that are ready for implementation. In this chapter, those artifacts are a product spec, acceptance criteria, an ADR, and an eval seed. The running example remains `FEATURE-001` in `sample-repo`.

## Learning Objectives
- Turn ambiguous requests into specs that include acceptance criteria and eval seeds
- Hand off finalized artifacts from an interactive assistant to a coding agent
- Separate the decisions humans must keep from the issue-framing work AI can accelerate

## Outline
### 1. Turn ambiguous requests into specs
### 2. Break requirements into acceptance criteria and eval seeds
### 3. Compare alternatives and make design decisions
### 4. Explain why ADRs matter
### 5. Divide responsibility among human, assistant, and agent
...
- Problem: support staff struggle to find tickets when they must guess whether the relevant text is in `title`, `description`, or `tags`
- Objective: let support staff narrow candidate tickets with one query
- In Scope: partial-match search over `title`, `description`, and `tags`
- Non-goals: ranking, typo correction, and external search infrastructure
...
The 2026 edition adds an eval seed at the same stage. That means the team also thinks about which later prompt or workflow evaluation will compare against this behavior. By keeping artifact criteria separate, acceptance includes not only runtime behavior but also whether docs, tests, and later eval cases stay synchronized.

Interactive assistants can generate many candidate acceptance criteria. Humans still need to decide which ones belong in the issue. Too many criteria expand implementation needlessly. Too few criteria leave completion subjective. At the CH03 stage, the practical filter is simple: each criterion should be easy to map to a test, review check, or later eval case.
...
## 5. Divide Responsibility Among Human, Assistant, and Agent
An interactive assistant is useful for decomposing vague requests, proposing candidate specs, and organizing trade-offs. It should not silently own decisions that carry scope, complexity, or accountability.

Humans still need to keep at least the following decisions.
...
5. which finalized artifacts should be handed off to the coding agent
...
That boundary makes exploratory work faster. Humans use ChatGPT to find the questions and compare options, then convert only the approved decisions into repo artifacts. The next coding agent should receive the product spec, acceptance criteria, ADR, and eval seed, not the full exploratory transcript.
...
Use ChatGPT first to surface the user, the problem, the in-scope behavior, the non-goals,
...
Compare two design options and preserve the selected option with rationale in
`sample-repo/docs/design-docs/ticket-search-adr.md`.
```
...
- The good version gives the product spec, acceptance criteria, and ADR separate roles.
- The good version also shows where human approval enters the flow.
...
3. Rewrite the spec as acceptance criteria and an eval seed
...
## Chapter Summary
- ChatGPT is most useful when it turns ambiguous requests into product specs, acceptance criteria, ADRs, and eval-seed-ready artifacts rather than trying to implement directly from a vague request.
- Exploratory dialogue and implementation-ready artifacts are different things. The coding agent should receive the finalized artifacts, not the full conversation.
- Once the spec artifacts are stable, the next question is whether the same prompt and workflow produce stable quality repeatedly. CH04 addresses that with eval cases, rubrics, and trace-aware comparison.

## Parity Notes
- Japanese source: `manuscript/part-01-prompt/ch03-requirements-and-design.md`
- This English draft preserves the eval-seed addition and the explicit handoff boundary among human, interactive assistant, and coding agent.
