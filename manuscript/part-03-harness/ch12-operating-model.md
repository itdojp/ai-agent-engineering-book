...
- mergeからproduction確認、停止、rollback、再開までのgateを設計できる
...
AIエージェント運用では、PR が「CI green だから完了」になりやすい。だが Phase 2 の GitHub / AgentOps 系書籍と揃えるなら、完了ゲートには review body、inline comment、suggestion、未解決 review thread、CI、merge 後確認を含める。`.github/pull_request_template.md` は、Copilot review を依頼したか、全コメントに修正または不要理由で返信したか、未解決 thread が 0 か、merge 後の main checks と公開反映を確認したかを記録できる形にしておく。

この `review completion gate` は、reviewer を増やすためではなく、review budget を守るための gate である。未返信の suggestion や unresolved thread を残したまま merge すると、post-merge regression の原因が review 漏れなのか、verify 漏れなのか、仕様判断なのか分からなくなる。Operator は PR body に current-run verify と review-response evidence を残し、Reviewer は unresolved thread 0 と CI green を merge 判断の前提にする。
...
deploymentがfailed/unknown、markerが不一致、代表routeが異常、またはmetricがthresholdを超えた場合は `Halted` とする。rollbackは履歴改変ではなく、review済みのrevert PR等による新しいmain commitを既定にする。過去runのrerunは元のSHA/refを使うため、mainと公開物を乖離させる既定rollbackには向かない。rollback後も新しいSHAに対して同じproduction確認を繰り返し、原因、是正、再検証、再開判断が揃ってから次へ進む。
...
metrics は「AIエージェントが便利か」を測るためではない。運用が健全か、どこが詰まっているかを判断するために使う。`docs/metrics.md` では、Throughput、Quality、Production Delivery、Hygiene、Observability の 5 群に分けている。

| 群 | 例 | 見たいこと |
|---|---|---|
| Throughput | closed issues / week、PR cycle time、review completion rate、stale draft count | work package と review queue が適切か |
| Quality | verify failure rate、approval-stop rate、post-merge regression count | review と verification harness が効いているか |
| Production Delivery | production confirmation rate/latency、marker mismatch、rollback recovery time | merge後の公開確認と復旧が閉じているか |
...
重要なのは、指標を増やすことではなく、指標に対する行動を決めることだ。verify failure rate が高ければ prompt や brief の質を疑う。PR cycle time が長ければ review budget を疑う。review completion rate が低い、または unresolved review thread residual が残るなら、merge 前 gate と PR template を疑う。production confirmation rateが100%未満、marker mismatchまたはpost-deploy metric regressionが発生した場合は、merge済みでも完了を止める。trace coverage が低ければ failure analysis の材料不足を疑う。evidence freshness failure が多ければ reviewer が current-run verify を確認できていないことを疑う。stale docs count と hygiene backlog age が悪化したら cleanup 専用 work package を先に開く。metrics は throughput 自慢ではなく、queue 診断、failure analysis、review quality 改善に使うべきである。
...
Copilot review、inline comment / suggestion への返信、未解決 thread 0、CI green、merge 後確認を完了ゲートにする。
productionへ影響する変更は、merge前にtarget/marker/metric/owner/rollbackを決め、merge後に対象SHAのdeployment、HTTP、semantic marker、metricを別々に確認する。
...
公開変更では、leadがmerge判断、operatorがdeploy証跡の収集、reviewerがreview completion、Delivery Ownerがproduction confirmationを担当する。1人運用なら同一人物が兼任しても、判断記録は分ける。成功scenarioでは対象SHAのdeployment success、root/代表routeのHTTP 200、semantic marker一致、metric閾値内を確認する。deploy failure/unknownなら完了を止める。markerまたはmetric回帰ならrevert PRでrollbackし、新しいmain SHAの公開確認まで行う。
...
- この章を探し直すときは、まず `docs/operating-model.md`、`docs/metrics.md`、`checklists/verification.md`、`checklists/repo-hygiene.md`、`.github/pull_request_template.md` を正本として見る。導入判断はモデル比較ではなく、役割、review budget、queue 診断、production gate、cleanup の設計で読む。
...
- throughput を上げるには、モデルを速くするより review budget、review completion gate、work package を整える方が先である。
- review completion、deployment approval、deployment success、production confirmationを分離し、対象SHAのHTTP、semantic marker、metricで完了を判定する。
